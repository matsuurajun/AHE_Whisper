...
class Diarizer:
    ...
    def get_speaker_probabilities(
        ...
        # --- [DEBUG-UPPER-BOUND] Optional dump for CampPlus+DP upper-bound experiment ---
        # 環境変数 AHE_WHISPER_SPK_PROBS_NPZ が指定されている場合のみ、
        # grid_times ベースの spk_probs を npz として保存する。
        dump_path = os.environ.get("AHE_WHISPER_SPK_PROBS_NPZ", "").strip()
        if dump_path:
            try:
                # grid_times は DP／評価で使うフレーム時刻そのものなので、
                # これをそのまま upper-bound スクリプト側の frame_times として扱う。
                frame_times = np.asarray(grid_times, dtype=np.float32)
                ...
                np.savez(
                    ...
                    spk_probs=spk_probs.astype(np.float32, copy=False),
                    duration_sec=float(frame_times[-1]) if frame_times.size > 0 else 0.0,
                )
                LOGGER.info(
                    "[SPK-PROBS] debug upper-bound dump saved to %s (frames=%d, speakers=%d)",
                    str(out_path),
                    frame_times.size,
                    spk_probs.shape[1],
                )
                ...
                LOGGER.warning(
                    "[SPK-PROBS] failed to dump upper-bound npz to %s: %s",
                    dump_path,
                    e,
                )
